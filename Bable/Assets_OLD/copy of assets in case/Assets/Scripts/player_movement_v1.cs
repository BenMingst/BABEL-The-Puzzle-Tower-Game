...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Dropdown")]
    public Collider2D[] dropdownColliders;
    ...
    void Start()
    {
        ...
        // auto-find all dropdown colliders in current scene by layer
        int dropdownLayer = LayerMask.NameToLayer("Dropdown");
        if (dropdownLayer != -1)
        {
            Collider2D[] allColliders = FindObjectsByType<Collider2D>(FindObjectsSortMode.InstanceID);
            List<Collider2D> found = new List<Collider2D>();
            foreach (var col in allColliders)
            {
                if (col.gameObject.layer == dropdownLayer)
                    found.Add(col);
            }
            dropdownColliders = found.ToArray();
        }
    }
    ...
    void IgnoreDropdownCollisions(bool ignore)
    {
        if (dropdownColliders == null) return;
        foreach (var dc in dropdownColliders)
        {
            if (dc == null) continue;
            if (standingCollider != null) Physics2D.IgnoreCollision(standingCollider, dc, ignore);
            if (crouchingCollider != null) Physics2D.IgnoreCollision(crouchingCollider, dc, ignore);
            if (rollingCollider != null) Physics2D.IgnoreCollision(rollingCollider, dc, ignore);
        }
    }
    ...
    void Update()
    {
        ...
        {
            IgnoreDropdownCollisions(true);
        }
        else if (!isDropping && isGrounded && !isInsideDropdown)
        {
            IgnoreDropdownCollisions(false);
        }
